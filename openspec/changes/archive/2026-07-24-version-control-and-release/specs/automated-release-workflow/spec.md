...
### Requirement: Template workflow uses dynamic version
The workflow template generated by `--init` SHALL reference the package version dynamically, not a hardcoded version string.

#### Scenario: Template does not contain hardcoded version
- **WHEN** `repository-hygiene --init .` generates `.github/workflows/repository-hygiene.yml`
- **THEN** the workflow SHALL NOT contain a hardcoded version string (e.g., `repository-hygiene==0.2.0`)
- **AND** the install command SHALL reference `repository-hygiene` without a fixed version number
