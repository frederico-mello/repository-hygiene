...
- **Manter códigos de saída.** Achados de severidade `error` continuam retornando `1`; auditoria sem erros retorna `0`; qualquer erro de configuração, execução ou persistência retorna `2`. O código `2` deliberadamente agrupa caminho de saída inválido, diretório pai ausente e falha de I/O para manter a convenção existente de erro de execução.
